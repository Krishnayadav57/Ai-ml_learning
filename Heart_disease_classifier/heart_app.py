...
model = joblib.load(r"F:\AI_ML_classes\Heart_disease_classifier\heart_disease_classifier.pkl")

#scaler = joblib.load("scaler_heart.pkl")
scaler = joblib.load("F:\AI_ML_classes\Heart_disease_classifier\scaler_heart.pkl")

#model_columns = joblib.load("model_columns.pkl")
model_columns = joblib.load("F:\AI_ML_classes\Heart_disease_classifier\model_columns.pkl")
...
resting_bp = st.number_input("Resting_Blood_Pressure", min_value=50, max_value=250, value=120)
cholesterol = st.number_input("Cholesterol", min_value=100, max_value=600, value=200)
max_hr = st.number_input("Max_Heart_Rate", min_value=50, max_value=220, value=150)
st_depression = st.number_input("ST_Depression", min_value=0.0, max_value=10.0, value=1.0)
...
if st.button("Predict"):
    ...
    input_encoded = pd.get_dummies(input_data, columns = ['Gender','Chest_Pain_Type','Fasting_Blood_Sugar','Resting_ECG','Exercise_Induced_Angina','ST_Slope','Number_of_Vessels','Thalassemia'], drop_first=False, dtype=int)
